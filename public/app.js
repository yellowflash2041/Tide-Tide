...
ngApp.run(function ($rootScope, $http) {
  console.log(
    "%capp.js loaded",
    "background-color:lightgreen; color:black; font-size:12px"
  );
...
ngApp.controller("allbooks", function ($scope, $rootScope, $http) {
  ...
  $scope.$on("$stateChangeSuccess", function () {
    ...
    $http(req).then(
      ...
      function (res) {
        //Error function text is in res.data
        $scope.pollingCompleted = true;
        $scope.error = true;
  ...
  $scope.tradeBook = function ($event) {
    ...
    if (typeof book == "undefined" || typeof book._id == "undefined") {
      console.log(
        "%cUnable to find book data in JSON",
...
ngApp.controller("addbook", function ($scope, $http, $state) {
    ...
    $http.get(queryURI).then(
      function (response) {
        if (response.data.hasOwnProperty("items")) {
          ...
          //Not an error, but no results returned.  There sill not be a response.data.error.message, as this is not an error but simply just no results...
  ...
  $scope.updateCoverPreview = function () {
    ...
    if (!$scope.book.hasOwnProperty("selectedBook")) {
      console.error(
        "%cExpected $scope to have property 'selectedBook'.",
        "background-color:black; color:red; font-size:12px"
      );
      return false;
    }
    if (isNaN(Number($scope.book.selectedBook))) {
      console.error(
        "%cUnabled to cast 'selectedBook' to Number.",
        "background-color:black; color:red; font-size:12px"
      );
      return false;
    }
    ...
    if (
      typeof $scope.searchResults.items[selectedBookIndex].volumeInfo
        .imageLinks !== "undefined"
    ) {
      var img2use =
        $scope.searchResults.items[selectedBookIndex].volumeInfo.imageLinks
          .thumbnail;
      $scope.book.img2use = img2use;
      $scope.book.title =
        $scope.searchResults.items[selectedBookIndex].volumeInfo.title;
    } else {
  ...
  $scope.addBook = function () {
    ...
    var book = { title: $scope.book.title, image: $scope.book.img2use };

    $http({ method: "POST", url: "/addnewbook", data: book }).then(
      function (response) {
        if (response.data.status == "added") {
          abBtn
            .removeClass("adding-button")
            .addClass("add-success-button")
            .text("Success!");
          setTimeout(function () {
            $state.go("mybooks");
          }, 5000);
      ...
      function (response) {
        ...
        window.alert(
          "There was an error processing your request: " + response.data
        );
        abBtn
          .removeClass("adding-button")
          .addClass("add-error-button")
          .text("Error");
      }
    );
  };
...
ngApp.controller("TradeBtnsCtrl", function ($scope, $http, $rootScope) {
  ...
  $scope.removeTradeEle = function (tradeDiv) {
    if (
      tradeDiv.tagName == "DIV" &&
      angular.element(tradeDiv).scope().hasOwnProperty("trade")
    ) {
  ...
  $scope.showUser = function (event, user) {
    ...
    if ($scope.lastShowUserTarget == event.target) {
      ...
      $http({ url: "getProfile/" + user.userName }).then(function (res) {
        ...
        var fullname =
          res.data.fullname == null ? "<i>(empty)</i>" : res.data.fullname;
        var profInfo = angular.element(
          `<div>${user.userName} - Full Name: ${fullname} - City: ${city} - State: ${state}</div>`
        );
